# script for model averaging at each spatial extent
...
extract_model_average_data <- function(model_average_object, model_name, extent, param_name) {

  # extract items from the model output
  extent <- extent
  model <- model_name
  parameter <- param_name
  beta_est <- model_average_object$Mod.avg.beta
  st_error <- model_average_object$Uncond.SE
  conf_level <- model_average_object$Conf.level
  lower_cl <- model_average_object$Lower.CL
  upper_cl <- model_average_object$Upper.CL
...
extract_null_summary <- function(model, extent, model_name) {
...
invert_biomass_models <- function(data, extent) {
  ...
  if (extent == "local") {
    invert_biomass_null_model <- lm(invertebrate_biomass ~ 1, data)

    invert_biomass_model10 <- lm(invertebrate_biomass ~ total_road_density, data)
    ...
    model_table <- extract_model_average_data(averaged_model, "invertebrate_biomass", extent, param_name)
  ...
  if (extent == "riparian" || extent == "catchment") {
    ...
    model_table <- extract_null_summary(model_list, extent, "invertebrate_biomass")
  }
...
ept_index_models <- function(data, extent) {
  ...
  if (extent == "local") {
    ...
    ept_index_null_model <- lm(ept_index ~ 1, data)

    ept_index_model9 <- lm(ept_index ~ total_disturbance, data)
    ...
    averaged_model <- modavg(cand.set = model_list, parm = param_name)
    model_table <- extract_model_average_data(averaged_model, "ept_index", extent, param_name)
  }
  ...
  if (extent == "riparian") {
    ...
    model_table1 <- extract_model_average_data(averaged_model1, "ept_index", extent, param_name1)

    param_name2 <- "total_disturbance"
    averaged_model2 <- modavg(cand.set = model_list, parm = param_name2)
    model_table2 <- extract_model_average_data(averaged_model2, "ept_index", extent, param_name2)

    model_table <- bind_rows(model_table1, model_table2)
    View(averaged_model1)
  }
  ...
  if (extent == "catchment") {
    ...
    # extract data from the null model
    model_table <- extract_null_summary(model_list, extent, "ept_index")
  }
...
periphyton_models <- function(data, extent) {
  ...
  if (extent == "local" || extent == "riparian" || extent == "catchment") {
    ...
    periphyton_null_model <- lm(periphyton_biomass ~ 1, data)

    model_list <- periphyton_null_model

    # extract data from the null model
    model_table <- extract_null_summary(model_list, extent, "periphyton_biomass")
  }
...
shredder_models <- function(data, extent) {
  ...
  if (extent == "local") {
    ...
    shredder_model1 <- lm(shredder ~ total_disturbance + wetted_width + wolmanD50, data)
    ...
    param_name1 <- "total_road_density"
    averaged_model1 <- modavg(cand.set = model_list, parm = param_name1)
    model_table1 <- extract_model_average_data(averaged_model1, "shredders", extent, param_name1)

    param_name2 <- "total_disturbance"
    averaged_model2 <- modavg(cand.set = model_list, parm = param_name2)
    model_table2 <- extract_model_average_data(averaged_model2, "shredders", extent, param_name2)

    param_name3 <- "wetted_width"
    averaged_model3 <- modavg(cand.set = model_list, parm = param_name3)
    model_table3 <- extract_model_average_data(averaged_model3, "shredders", extent, param_name3)

    param_name4 <- "wolmanD50"
    averaged_model4 <- modavg(cand.set = model_list, parm = param_name4)
    model_table4 <- extract_model_average_data(averaged_model4, "shredders", extent, param_name4)

    param_name5 <- "high_human_impact"
    averaged_model5 <- modavg(cand.set = model_list, parm = param_name5)
    model_table5 <- extract_model_average_data(averaged_model5, "shredders", extent, param_name5)

    model_table <- bind_rows(model_table1, model_table2, model_table3, model_table4, model_table5)
  }
  ...
  if (extent == "riparian") {
    ...
    model_table <- bind_rows(model_table1, model_table2, model_table3, model_table4)
  }
...
nitrogen_models <- function(data, extent) {
  ...
  if (extent == "local" || extent == "riparian" || extent == "catchment") {
    ...
    total_nitrogen_null_model <- lm(tn ~ 1, data)

    model_list <- total_nitrogen_null_model

    # extract data from the null model
    model_table <- extract_null_summary(model_list, extent, "nitrogen")
  }
...
conductivity_models <- function(data, extent) {
  ...
  if (extent == "local") {
    ...
    specific_conductivity_null_model <- lm(conductivity ~ 1, data)

    model_list <- specific_conductivity_null_model

    # extract data from the null model
    model_table <- extract_null_summary(model_list, extent, "specific_conductivity")
  }
  ...
  if (extent == "riparian") {
    ...
    averaged_model1 <- modavg(cand.set = model_list, parm = param_name1)
    model_table1 <- extract_model_average_data(averaged_model1, "specific_conductivity", extent, param_name1)

    param_name2 <- "high_human_impact"
    averaged_model2 <- modavg(cand.set = model_list, parm = param_name2)
    model_table2 <- extract_model_average_data(averaged_model2, "specific_conductivity", extent, param_name2)
...
embeddedness_models <- function(data, extent) {
  ...
  if (extent == "local" || extent == "riparian" || extent == "catchment") {
    ...
    percent_embeddedness_null_model <- lm(embeddedness ~ 1, data)

    model_list <- percent_embeddedness_null_model

    # extract null summary
    model_table <- extract_null_summary(model_list, extent, "embeddedness")
  }
...
# use run all models function at each extent
...
write.csv(formatted_model_output, "output/model_averaging_formatted.csv", row.names = FALSE)

# now plot the model averaged data --------------------------------------
...
p1 <- ggplot(plotting_data_local, aes(parameter,
  ...
  geom_errorbar(aes(ymin = beta_est - st_error, ymax = beta_est + st_error), width = .2,
...
# export plots
ggsave(file="output/model_summary_local_averaged.svg", plot=p1, width=8, height=2)
ggsave(file="output/model_summary_riparian_averaged.svg", plot=p2, width=8, height=2)
ggsave(file="output/model_summary_catchment_averaged.svg", plot=p3, width=6, height=1.75)
